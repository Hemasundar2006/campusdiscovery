...
// Utility to format date for input field (YYYY-MM-DD)
    ...
    const fetchEvent = async () => {
      try {
        ...
        setFormData({
          ...
          lastDate: formatDateForInput(event.lastDate),
          eventDates: event.eventDates || '',
        });
        setQuery(event.location?.address || '');
        setCompetitions(event.competitions || []);
      } catch (err) {
    ...
    <div className="min-h-screen bg-[#f8fafc] py-32 px-6">
      <div className="max-w-4xl mx-auto">
        <header className="mb-16">
          <button onClick={() => navigate(-1)} className="text-[11px] font-bold uppercase tracking-widest text-emerald-600 mb-4">← Back</button>
          <h1 className="text-5xl md:text-6xl font-semibold tracking-tight text-slate-900 italic">Edit <span className="text-emerald-600">Event.</span></h1>
          <p className="text-slate-500 font-medium text-lg mt-4">Update your fest details and re-broadcast.</p>
        </header>
        ...
        <form onSubmit={handleSubmit} className="space-y-12">
          <div className="bg-white rounded-[3.5rem] p-10 md:p-14 border border-slate-50 shadow-sm space-y-8">
            ...
            <div className="space-y-2">
              <label className="text-[11px] font-bold uppercase tracking-widest text-slate-400 ml-4">Update Visual (Leave blank to keep current)</label>
              <input 
                type="file"
                accept="image/*"
                className="w-full bg-slate-50 border border-slate-100 rounded-[1.5rem] px-8 py-5 focus:border-emerald-500 outline-none transition-all font-medium"
          ...
          <button type="submit" disabled={saving} className="w-full bg-slate-900 text-white py-6 rounded-[2rem] font-bold uppercase tracking-[0.2em] hover:bg-emerald-600 transition-all shadow-xl shadow-slate-900/10">
            {saving ? 'Updating Broadcast...' : 'Save & Publish Changes'}
          </button>
        </form>
      </div>
    </div>
